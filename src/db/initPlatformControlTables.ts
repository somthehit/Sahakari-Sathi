...
async function safe(db: any, label: string, query: any) {
  try {
    await db.execute(query);
  } catch (err: any) {
    console.error(`[DB] Init skip ${label}: ${err?.cause?.code || err?.message || err}`);
  }
}

export async function initPlatformControlTables(): Promise<void> {
  const db = getDb();
  if (!db) return;

  // ── subscription_plans ──
  await safe(db, 'subscription_plans', sql`
  ...
  `);
  await safe(db, 'splans_code_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'splans_code_idx') THEN CREATE UNIQUE INDEX splans_code_idx ON subscription_plans(code); END IF; END $$;`);
  await safe(db, 'splans_status_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'splans_status_idx') THEN CREATE INDEX splans_status_idx ON subscription_plans(status); END IF; END $$;`);
  await safe(db, 'splans_sort_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'splans_sort_idx') THEN CREATE INDEX splans_sort_idx ON subscription_plans(sort_order); END IF; END $$;`);

  // ── system_settings ──
  await safe(db, 'system_settings', sql`
  ...
  `);
  await safe(db, 'syscfg_key_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'syscfg_key_idx') THEN CREATE UNIQUE INDEX syscfg_key_idx ON system_settings(key); END IF; END $$;`);
  await safe(db, 'syscfg_cat_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'syscfg_cat_idx') THEN CREATE INDEX syscfg_cat_idx ON system_settings(category); END IF; END $$;`);

  // ── api_keys ──
  await safe(db, 'api_keys', sql`
  ...
  `);
  await safe(db, 'apikey_prefix_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'apikey_prefix_idx') THEN CREATE INDEX apikey_prefix_idx ON api_keys(key_prefix); END IF; END $$;`);
  await safe(db, 'apikey_status_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'apikey_status_idx') THEN CREATE INDEX apikey_status_idx ON api_keys(status); END IF; END $$;`);
  await safe(db, 'apikey_org_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'apikey_org_idx') THEN CREATE INDEX apikey_org_idx ON api_keys(organization_id); END IF; END $$;`);

  // ── platform_announcements ──
  await safe(db, 'platform_announcements', sql`
  ...
  `);
  await safe(db, 'pannc_active_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'pannc_active_idx') THEN CREATE INDEX pannc_active_idx ON platform_announcements(is_active); END IF; END $$;`);
  await safe(db, 'pannc_type_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'pannc_type_idx') THEN CREATE INDEX pannc_type_idx ON platform_announcements(type); END IF; END $$;`);
  await safe(db, 'pannc_priority_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'pannc_priority_idx') THEN CREATE INDEX pannc_priority_idx ON platform_announcements(priority); END IF; END $$;`);
  await safe(db, 'pannc_audience_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'pannc_audience_idx') THEN CREATE INDEX pannc_audience_idx ON platform_announcements(target_audience); END IF; END $$;`);

  // ── database_settings ──
  await safe(db, 'database_settings', sql`
  ...
  `);
  await safe(db, 'dbcfg_env_idx', sql`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'dbcfg_env_idx') THEN CREATE INDEX dbcfg_env_idx ON database_settings(environment); END IF; END $$;`);

  // ── Seed defaults ──
  try {
    const settingsCount = await db.execute<{ cnt: number }>(sql`SELECT COUNT(*)::int AS cnt FROM system_settings`);
    const sc = Array.isArray(settingsCount) ? settingsCount : (settingsCount as any)?.rows ?? [];
    if ((sc[0]?.cnt ?? 1) === 0) {
      ...
      `);
      console.log('[DB] Default system_settings seeded.');
    }
  } catch (err: any) {
    console.error('[DB] Init skip settings seed:', err?.cause?.code || err?.message);
  }

  try {
    const plansCount = await db.execute<{ cnt: number }>(sql`SELECT COUNT(*)::int AS cnt FROM subscription_plans`);
    const pc = Array.isArray(plansCount) ? plansCount : (plansCount as any)?.rows ?? [];
    if ((pc[0]?.cnt ?? 1) === 0) {
      await db.execute(sql`
      ...
      `);
      console.log('[DB] Default subscription_plans seeded.');
    }
  } catch (err: any) {
    console.error('[DB] Init skip plans seed:', err?.cause?.code || err?.message);
  }

  console.log('[DB] platform_control tables initialized.');
}
