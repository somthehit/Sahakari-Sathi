...
import { Minus, Download, FileSpreadsheet, Printer, Loader2, RefreshCw, Filter } from 'lucide-react';
import { formatNPR } from '../../../utils/nepaliCalendar';
import { exportToPdf, exportToExcel } from '../../../utils/exportUtils';
import { fetchShareTransactions, type ShareTransaction } from '../../../api/shares';

export const ShareReturnReport: React.FC = () => {
  ...
  const load = async () => {
    try {
      ...
      setData(all.filter(t => t.transactionType === 'Surrender' || t.transactionType === 'Transfer_Out'));
    } catch (e: any) {
  ...
  const handleExportPdf = () => {
    const headers = ['Date (BS)', 'Member No', 'Member Name', 'Share Type', 'Type', 'Shares', 'Amount', 'Voucher No'];
    const rows = filtered.map(t => [t.dateBs, t.memberNo || '-', t.memberName || '-', t.shareTypeName || '-', t.transactionType, t.numberOfShares, t.totalAmount, t.voucherNo || '-']);
    exportToPdf('Share_Return_Report', 'Share Return / Surrender Report', `Total: ${stats.totalReturns} returns | ${stats.totalShares} shares`, headers, rows);
  };

  const handleExportExcel = () => {
    const headers = ['Date (BS)', 'Member No', 'Member Name', 'Share Type', 'Type', 'Shares', 'Amount', 'Voucher No'];
    const rows = filtered.map(t => [t.dateBs, t.memberNo || '-', t.memberName || '-', t.shareTypeName || '-', t.transactionType, t.numberOfShares, t.totalAmount, t.voucherNo || '-']);
    exportToExcel('Share_Return_Report', 'Share_Returns', headers, rows);
  };
  ...
  return (
    <div className="p-1 sm:p-2.5 space-y-4 max-w-[1800px] mx-auto text-slate-800 text-xs">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight">Share Return Report</h1>
          <p className="text-xs text-slate-500 mt-1 flex items-center gap-2 font-medium">
            <Minus className="w-3.5 h-3.5 text-slate-500" />
            Share surrender and return transactions — shares returned by members
          </p>
        </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center p-12"><Loader2 className="w-6 h-6 animate-spin text-emerald-600" /><span className="ml-2 text-sm text-slate-500">Loading Share Returns…</span></div>
      ) : error ? (
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              { label: 'Total Returns', value: String(stats.totalReturns), color: 'rose' },
              { label: 'Shares Returned', value: String(stats.totalShares), color: 'amber' },
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {filtered.map((t, idx) => (
                    <tr key={t.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="p-3">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${t.transactionType === 'Surrender' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'}`}>
                          {t.transactionType}
                        </span>
                      </td>
                      <td className="p-3 text-right font-mono font-semibold text-amber-700">{t.numberOfShares}</td>
                      <td className="p-3 text-right font-mono font-semibold text-rose-700">{formatNPR(t.totalAmount)}</td>
                      <td className="p-3 font-mono text-slate-500">{t.voucherNo || '-'}</td>
                    </tr>
                  ))}
                  {filtered.length === 0 && <tr><td colSpan={9} className="p-8 text-center text-slate-400">No share return transactions found.</td></tr>}
                </tbody>
                {filtered.length > 0 && (
                  <tfoot className="bg-slate-50 border-t-2 border-slate-200 font-bold text-slate-900">
                    <tr>
                      ...
                      <td className="p-3 text-right font-mono text-amber-700">{stats.totalShares}</td>
                      <td className="p-3 text-right font-mono text-rose-700">{formatNPR(stats.totalAmount)}</td>
                      <td></td>
                    </tr>
                  </tfoot>
                )}
