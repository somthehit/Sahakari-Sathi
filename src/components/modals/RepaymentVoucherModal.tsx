...
export const RepaymentVoucherModal: React.FC<Props> = ({ result, onClose }) => {
  const orgName = useAuthStore((s) => s.user?.organizationName || 'Sahakari Sathi');

  const handlePrint = (copyLabel: string) => {
    const content = document.getElementById(`repayment-voucher-${copyLabel.replace(/\s/g, '')}`);
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Repayment Voucher - ${result.receiptNo} - ${copyLabel}</title>
  ...
  const renderVoucher = (copyLabel: string) => (
    <div id={`repayment-voucher-${copyLabel.replace(/\s/g, '')}`} className="space-y-4 text-xs">
      {/* Header */}
      <div className="text-center border-b-2 border-emerald-600 pb-3">
        ...
        <h2 className="text-base font-extrabold text-emerald-800 mt-0.5">EMI Repayment Receipt</h2>
        <div className="text-[10px] text-slate-500 mt-0.5">{result.branchName} • {result.branchAddress}</div>
        <div className="inline-block mt-2 px-4 py-1 border border-emerald-300 rounded text-emerald-700 font-mono text-[10px] font-bold uppercase tracking-widest">
          {copyLabel}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          <div className="text-[10px] text-slate-500 uppercase font-semibold">Receipt No</div>
          <div className="font-mono font-bold text-emerald-700">{result.receiptNo}</div>
        </div>
        <div className="space-y-1">
          <div className="text-[10px] text-slate-500 uppercase font-semibold">Voucher No</div>
          <div className="font-mono font-bold text-slate-800">{result.voucherNo}</div>
        </div>
        <div className="space-y-1">
      ...
      <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1">
            ...
            <div className="font-mono font-bold text-emerald-700">{result.loanNo}</div>
            <div className="text-slate-600">Payment: {result.paymentMode}</div>
          </div>
        </div>
      </div>
      ...
      <div className="border-2 border-emerald-200 rounded-lg overflow-hidden">
        <div className="bg-emerald-600 text-white px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider">
          Payment Breakdown
        </div>
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-emerald-50 border-b border-emerald-200">
              <th className="text-left px-3 py-1.5 font-bold text-emerald-900">Description</th>
              <th className="text-right px-3 py-1.5 font-bold text-emerald-900">Amount (NPR)</th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-b border-slate-100">
              <td className="px-3 py-1.5 text-slate-600">Principal Repayment</td>
              <td className="px-3 py-1.5 text-right font-mono font-bold text-emerald-700">{result.principalPortion.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
            </tr>
            <tr className="border-b border-slate-100">
              <td className="px-3 py-1.5 text-slate-600">Interest Payment</td>
              <td className="px-3 py-1.5 text-right font-mono font-bold text-amber-700">{result.interestPortion.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
            </tr>
          </tbody>
          <tfoot>
            <tr className="bg-emerald-50 border-t-2 border-emerald-300 font-bold">
              <td className="px-3 py-2 text-emerald-900">Total Paid</td>
              <td className="px-3 py-2 text-right font-mono text-emerald-900 text-sm">{result.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
        <div className="flex justify-between items-center">
          <span className="text-slate-600 font-semibold">Outstanding Principal After Payment</span>
          <span className="font-mono font-bold text-lg text-slate-900">
            NPR {result.outstandingAfter.toLocaleString(undefined, { minimumFractionDigits: 2 })}
          </span>
        </div>
        {result.closed && (
          <div className="mt-2 flex items-center gap-1.5 text-emerald-700 font-bold">
            <CheckCircle2 className="w-4 h-4" />
            Loan Fully Closed — No Outstanding Balance
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 pt-6 text-[10px] text-slate-500 text-center">
        <div>
          <div className="border-t border-slate-400 pt-1.5 mt-6 font-semibold">Received By</div>
          <div className="mt-0.5 font-bold text-slate-700">{result.collectedBy}</div>
        </div>
        <div>
      ...
      <div className="text-center text-[9px] text-slate-400 pt-2">
        Printed: {new Date().toLocaleString()} • This is a computer-generated receipt.
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-sm text-slate-900">Repayment Posted Successfully</h3>
              <p className="text-[11px] text-slate-500">Receipt: {result.receiptNo} • Voucher: {result.voucherNo}</p>
            </div>
          </div>
        ...
        <div className="p-4 bg-emerald-50 border-b border-emerald-100">
          <div className="text-center">
            <div className="text-[10px] text-emerald-600 uppercase font-semibold">Amount Received</div>
            <div className="text-2xl font-extrabold text-emerald-800 font-mono mt-0.5">
              NPR {result.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </div>
            {result.closed && (
              <div className="text-xs text-emerald-700 font-bold mt-1 flex items-center justify-center gap-1">
        ...
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl transition cursor-pointer shadow-sm text-xs"
            >
